...
"""
Extracts names and dates from a dictionary of musicians. Originally written to process Baker's Biographical Dictionary of Musicians, but should work with any dictionary with similar formatting.
"""
...
def get_dates(text):
	...
	x=re.search("[.,;\-\(\) ]([bh][\.,]|born ).{,%s}?[.,;\-\(\) ](?P<birth>[1lIi][0-9lJIgiSo]{3})[.,;\-\(\) ]" %(maxchar,), text)
	birth=x.group("birth") if x else "?"
	y=re.search("[.,;\-\(\) ](d[\.,]|died ).{,%s}?[.,;\-\(\) ](?P<death>[1lIi][0-9lJIgiSo]{3})[.,;\-\(\) ]" %(maxchar,), text)
